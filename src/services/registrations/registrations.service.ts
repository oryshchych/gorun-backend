...
function formatRegistrationResponse(registration: RegistrationDoc): RegistrationResponse {
...
export async function createRegistration(
  ...
  try {
    ...
    await registration.populate([{ path: 'event' }, { path: 'user', select: 'name email image' }]);

    return formatRegistrationResponse(registration.toObject());
  } catch (error) {
...
export async function createPublicRegistration(
  ...
  try {
    ...
    if (existing) {
      ...
      if (existing.paymentStatus === 'pending' || existing.paymentStatus === 'failed') {
        ...
        if (finalPrice === 0) {
          ...
          return {
            registration: formatRegistrationResponse(existing.toObject()),
    ...
    if (isFreeRegistration) {
      ...
      return {
        registration: formatRegistrationResponse(registration.toObject()),
        // No paymentLink for free registrations
      };
    }
        ...
        if (keys.includes('email')) {
          ...
          if (duplicateRegistration) {
            ...
            if (
              ...
              const response: { registration: RegistrationResponse; paymentLink?: string } = {
                registration: formatRegistrationResponse(duplicateRegistration.toObject()),
              };
              if (paymentLink) {
                response.paymentLink = paymentLink;
              }
 ...
 */
export async function cancelRegistration(id: string, userId: string): Promise<void> {
...
export async function getRegistrations(
  ...
  const registrationResponses = registrations.map(reg => formatRegistrationResponse(reg));
 ...
 */
export async function getPaymentLinkByEmail(
  email: string,
  eventId: string
 ...
 */
export async function syncPaymentStatus(registrationId: string): Promise<{
  ...
  const payment = await Payment.findById(registration.paymentId);
  if (!payment) {
    throw new NotFoundError('Payment not found', REGISTRATIONS_CODES.ERROR_REGISTRATION_NOT_FOUND);
  }
  ...
  if (isSuccess && payment.status !== 'completed') {
    ...
    return {
      registration: formatRegistrationResponse(
