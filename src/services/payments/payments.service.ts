 ...
 */
async function createPlataInvoice(params: {
  amount: number;
  customerName: string;
  eventTitle: string;
...
export async function createPaymentWithInvoice({
  ...
  const invoice = await createPlataInvoice({
    amount,
    customerName,
    eventTitle,
    registrationId,
 ...
 */
export async function findByInvoiceId(invoiceId: string): Promise<IPayment | null> {
 ...
 */
export async function getReceipt(paymentId: string): Promise<Record<string, unknown> | null> {
  const payment = await Payment.findById(paymentId);
  if (!payment) {
    throw new AppError('Payment not found', 404, PAYMENTS_CODES.ERROR_PAYMENT_NOT_FOUND);
  }
