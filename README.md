...
- **User Registration & Login**: Credentials with `firstName` / `lastName` / `phone` (E.164) or legacy `name`; bcrypt hashing
- **Remember me**: Optional `rememberMe` on login (and `remember_me` on Google start) — short vs long **refresh** JWT TTL (`JWT_REFRESH_EXPIRY` vs `JWT_REFRESH_EXPIRY_LONG`)
- **Google OAuth**: Server-side authorization code flow — `GET /api/auth/google` → callback → frontend receives one-time `code` → `POST /api/auth/oauth/exchange` returns the same JWT shape as login
- **Password reset**: `POST /api/auth/forgot-password` + `POST /api/auth/reset-password` (opaque token, email via Resend when configured)
- **JWT Token Management**: Access + refresh tokens; rotation on refresh preserves short/long session
...
| `JWT_REFRESH_EXPIRY`                                                      | Refresh token TTL (no remember-me / OAuth `remember_me=false`)             | `7d`                                        | Yes              |
| `JWT_REFRESH_EXPIRY_LONG`                                                 | Refresh token TTL (remember-me / OAuth `remember_me=true`)                 | `30d`                                       | Yes              |
| `PASSWORD_RESET_TOKEN_EXPIRY`                                             | Reset link token TTL                                                       | `1h`                                        | No               |
| `OAUTH_EXCHANGE_CODE_TTL_SEC`                                             | One-time OAuth exchange code TTL                                           | `60`                                        | No               |
| `OAUTH_STATE_TTL_MIN`                                                     | OAuth CSRF `state` document TTL                                            | `10`                                        | No               |
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_OAUTH_REDIRECT_URI` | Google OAuth (backend callback URL only)                                   | —                                           | For Google login |
| `FRONTEND_OAUTH_REDIRECT_ORIGINS`                                         | Comma-separated allowed origins for `?redirect_uri=` on `/api/auth/google` | `FRONTEND_URL` origin                       | No               |
| `FRONTEND_URL`                                                            | Base URL for reset links and OAuth fallbacks                               | `http://localhost:3000`                     | Yes              |
| `RESEND_API_KEY` / `RESEND_FROM_EMAIL`                                    | Transactional email (password reset)                                       | —                                           | For sending mail |
...
- `POST /api/auth/register` - Register (`firstName`, `lastName`, `phone` E.164, or legacy `name`)
...
- `POST /api/auth/forgot-password` - Request reset email (generic 200)
